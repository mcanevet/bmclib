...
// on the conflict. It shadows redfishwrapper.Client.SetBiosConfiguration.
//
// This uses ApplyBiosAttributesExact rather than the embedded client's SetBiosConfiguration. The
// latter (via gofish's Bios.UpdateBiosAttributesApplyAt) diffs the requested attributes against
// the Bios resource's *currently applied* state and silently omits anything that already matches
// it, without an error - including, disastrously, an attribute a caller explicitly wants
// reverted to its original value while a *different*, still-pending change from an earlier call
// in the same boot cycle holds something else for that same attribute. Since nothing has
// actually rebooted yet, "currently applied" never differs from the original value, so the
// diffing path sends no PATCH at all for that attribute, isPendingSettingsConflict never even
// gets a chance to see the real conflict, and the earlier call's now-wrong pending value is left
// in place (confirmed live: this is exactly what happened, silently, mid-chain).
// ApplyBiosAttributesExact always includes every requested attribute, for the first attempt and
// every recovery retry alike.
func (c *recoveringRedfishClient) SetBiosConfiguration(ctx context.Context, attrs map[string]string) error {
	...
	err := c.ApplyBiosAttributesExact(ctx, converted)
