...
// against a regression found live: gofish's UpdateBiosAttributesApplyAt (used by
// redfishwrapper.SetBiosConfiguration) diffs requested attributes against the *currently
// applied* Bios resource and silently drops anything that matches it. Since nothing has actually
// rebooted mid-recovery, "currently applied" never moves - so reverting an attribute back to a
// value that happens to equal it (a very normal thing for recovery to do, since it's often
// restoring an earlier value) would be dropped from the retry PATCH entirely if recovery used
// that same gofish path. Recovery must PATCH directly instead, always including every merged
// attribute regardless of whether it matches the current Bios resource.
func TestSetBiosConfiguration_PendingSettingsConflictRecoveryIncludesUnchangedAttributes(t *testing.T) {
	...
	mux.HandleFunc("/redfish/v1/Systems/System.Embedded.1/Bios", func(w http.ResponseWriter, r *http.Request) {
		require.Equal(t, http.MethodGet, r.Method)
		// The currently applied value already equals what this call will request - the exact
		// condition that triggered the regression.
		_, _ = w.Write([]byte(`{
			"@odata.id": "/redfish/v1/Systems/System.Embedded.1/Bios",
			"Id": "Bios",
			"Attributes": {"NewAttr": "Bar"},
			"@Redfish.Settings": {
				"SettingsObject": {"@odata.id": "/redfish/v1/Systems/System.Embedded.1/Bios/Settings"},
				"SupportedApplyTimes": ["OnReset"]
			}
	...
	// NewAttr: Bar is identical to the currently-applied value read from the Bios resource above.
	err = client.SetBiosConfiguration(context.Background(), map[string]string{"NewAttr": "Bar"})
	require.NoError(t, err)

	require.Len(t, patchBodies, 2)
	assert.Contains(t, patchBodies[1], `"SecureBootPolicy":"Standard"`, "expected the retry to preserve the already-pending attribute")
	assert.Contains(t, patchBodies[1], `"NewAttr":"Bar"`, "expected the retry to include the requested attribute even though it matches the currently-applied value")
}
