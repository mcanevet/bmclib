...
// setBiosConfiguration is Dell's single entry point for staging BIOS Setup attribute changes.
// Every Dell BIOS-backed feature that PATCHes a BIOS Setup attribute (SetHTTPBootURI, and the
// exported SetBiosConfiguration itself) calls this instead of
// c.redfishwrapper.SetBiosConfiguration, so all of them recover from iDRAC's
// one-pending-job-at-a-time limit instead of failing outright - see
// recoverFromPendingSettingsConflict.
//
// This uses redfishwrapper.ApplyBiosAttributesExact rather than SetBiosConfiguration. The latter
// (via gofish's Bios.UpdateBiosAttributesApplyAt) diffs the requested attributes against the
// Bios resource's *currently applied* state and silently omits anything that already matches it,
// without an error - including, disastrously, an attribute a caller explicitly wants reverted to
// its original value while a *different*, still-pending change from an earlier call in the same
// boot cycle holds something else for that same attribute. Since nothing has actually rebooted
// yet, "currently applied" never differs from the original value, so the diffing path sends no
// PATCH at all for that attribute, isPendingSettingsConflict never even gets a chance to see the
// real conflict, and the earlier call's now-wrong pending value is left in place (confirmed
// live: this is exactly what happened, silently, mid-chain). ApplyBiosAttributesExact always
// includes every requested attribute, for the first attempt and every recovery retry alike.
func (c *Conn) setBiosConfiguration(ctx context.Context, attrs map[string]string) error {
	converted := make(schemas.SettingsAttributes, len(attrs))
	for k, v := range attrs {
		converted[k] = v
	}

	err := c.redfishwrapper.ApplyBiosAttributesExact(ctx, converted)
...
func (c *Conn) recoverFromPendingSettingsConflict(ctx context.Context, newAttrs map[string]string) error {
	biosURL, err := c.redfishwrapper.SystemsBIOSOdataID(ctx)
	if err != nil {
		return fmt.Errorf("finding Bios resource to locate pending settings: %w", err)
	}

	resp, err := c.redfishwrapper.Get(biosURL)
	if err != nil {
	...
	pendingResp, err := c.redfishwrapper.Get(settingsURL)
	if err != nil {
	...
	// retrying blindly.
	return c.redfishwrapper.ApplyBiosAttributesExact(ctx, merged)
}
...
func (c *Conn) cancelPendingBIOSConfigJob(ctx context.Context) error {
	jobs, err := c.redfishwrapper.Jobs(ctx)
	if err != nil {
	...
	for _, j := range jobs {
		...
		delResp, err := c.redfishwrapper.Delete(j.ODataID)
		if err != nil {
